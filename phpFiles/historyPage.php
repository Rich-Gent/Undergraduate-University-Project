...
<?php
  ...
  {
    $_SESSION['message'] = "You must be signed in to view your video history";
    header('Location ../homepage');
    exit();
  }

  //fetch all videos that the user has watched
  $videoInfo = array();
  $stmt = $mysqli->prepare("SELECT Videos.UserID, Videos.VideoID, Videos.VName, Videos.Thumbnail, Videos.Views, Videos.Banned FROM History INNER JOIN Videos ON History.VideoID = Videos.VideoID WHERE History.UserID = ? ORDER BY History.HistoryTS DESC");
  $stmt->bind_param('i', $userID);
  $stmt->execute();
  $stmt->bind_result($uploaderUserID, $tmpVID, $tmpVName, $tmpThumbnail, $tmpViews, $tmpBanned);
  $stmt->store_result();
  while($stmt->fetch())
  {
...
<link rel="stylesheet" href="../../includes/historyPage.css"/>
<script>
...
function deleteVideo(div, vID)
{
  var ajax=new XMLHttpRequest();
  var formdata = new FormData();
  formdata.append("vID", vID);
  ajax.open("POST", "../../scripts/deleteSpecificScript.php");
  ajax.send(formdata);
  ajax.onreadystatechange = function()
  {
    if (ajax.readyState == 4 && ajax.status == 200)
    {
        ...
        document.getElementsByClassName("thumbnail")[div].style.display = "none";
...
<div id="container">
  <div class="center">
      ...
      if(!isset($video))
      {
        echo "<div id='thumbMessage'>Video history is empty. Start watching some videos!</div>";
      }
      else
      {
        echo "<form method='post' action='../../scripts/clearHistoryScript.php'>";
        echo "<input id=clearBtn type='submit'  value='Clear History'>";
        echo "</form>";
        foreach($videoInfo as $val)
        {
          if($val->banned == 0)
          {
            ...
            echo "<div class='delete' onclick='deleteVideo($i, $val->vID)'>&times;</div>\n";
            echo  "<img alt='videoImage' class='videoImage' src=" . $val->vThumbnail  . " onclick= 'location.href=" . "\"$page\"" . "'> \n
                   <div class='videoInfo' onclick= 'location.href=" . "\"$page\"" . "'> <b>".  $val->vName ." </b> </div>\n
                   <div class='views'onclick= 'location.href=" . "\"$page\"" . "'>Views:<b>" . $val->views . "</b> </div> \n
                </div>\n";
            $i++;
          }
        }
