...
<?php
  ...
  {
    $_SESSION['message'] = "You must be signed in to view your Favourites";
  ...
  $videoInfo = array();
  $stmt = $mysqli->prepare("SELECT Videos.UserID, Videos.VideoID, Videos.VName, Videos.Thumbnail, Videos.Views, Videos.Banned FROM Favourites INNER JOIN Videos ON Favourites.VideoID = Videos.VideoID WHERE Favourites.UserID = ?");
...
<link rel="stylesheet" href="../../includes/favouritesPage.css"/>
<script>
...
function deleteVideo(div, vID)
{
  ...
  ajax.open("POST", "../../scripts/deleteFavouriteScript.php");
  ajax.send(formdata);
  ajax.onreadystatechange = function()
  {
    if (ajax.readyState == 4 && ajax.status == 200)
...
<div id="container">
  <div class="center">
      ...
      <div class="vidThumbnails">
        <h2>Your favourite videos</h2>
      <?php
      if(!isset($video))
      {
        echo "<div id='thumbMessage'>Favourites are empty. Start liking some videos!</div>";
      }
      else
      {
