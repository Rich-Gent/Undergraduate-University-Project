...
<?php
  ...
  $userID = $_GET['uID'];
  $stmt = $mysqli->prepare("SELECT AccessLevel, Username, UserImage, About, RegisteredSince FROM Users WHERE UserID = ?");
  $stmt->bind_param("i", $userID);
  $stmt->execute();
  $stmt->store_result();
  $stmt->bind_result($accessLevel, $uploaderUsername, $uploaderThumbnail, $uploaderAbout, $regSince);
  $stmt->fetch();
  $stmt->close();
...
<link rel="stylesheet" href="../../includes/aboutUser.css"/>
<script>
...
<div id="container">
  <div class="center">
      ...
      <div id="section1">
          <img id="uploaderImage" src="<?php echo $uploaderThumbnail; ?>" alt="UserImage" style="width:300px;height:300px;border-radius:50%;">
          <div id="uploaderName"><?php echo $uploaderUsername; ?></div>
      </div>
      <div id="section2">
        <div id="rgrSinceTitle">Registered Since: </div>
        <div id="rgrSince"><?php echo $regSince; ?></div>
      </div>
      <div id="section3">
        <div id="userLevelTitle">User Level: </div>
        <?php
          if($accessLevel == "1")
          {
            echo "<div id='userLevel'> Administrator</div>";
          }
          else if($accessLevel == "2")
          {
            echo "<div id='userLevel'> Verified user</div>";
          }
          else
          {
            echo "<div id='userLevel'> Normal User</div>";
          }
        ?>
      </div>
      <div id="section4">
        <div id="aboutAreaTitle">User Bio:</div>
        <div id="aboutArea"><?php if(!isset($uploaderAbout)){ echo "This user has not made a bio yet!";}else{echo $uploaderAbout;} ?></div>
      </div>
      <div id="searchField">
          <h2>Search</h2>
      </div>
