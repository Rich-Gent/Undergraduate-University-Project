...
<?php
  ...
  $stmt = $mysqli->prepare("SELECT AccessLevel, Email, BannerImage, About, RegisteredSince, Upgrade FROM Users WHERE UserID = ?");
  $stmt->bind_param("i", $userID);
  $stmt->execute();
  $stmt->store_result();
  $stmt->bind_result($accessLevel, $email, $bannerImage, $about, $regSince, $upgrade);
  $stmt->fetch();
  $stmt->close();
?>
<html>
...
<link rel="stylesheet" href="../../includes/uSettings.css"/>
<script>
...
<div id="container">
  <div class="center">
      ...
      <form id="settingsContainer" action="../../scripts/uSettingsScript.php" method="post" enctype="multipart/form-data">
        <fieldset>
          <legend>Settings</legend>
          <div id="section1">
            <label id="nameBoxlbl" for="nameBox">Name: <?php echo $username; ?></label>  <a id="chgUsername" href="#" onclick="openText('nameBox')">Change Username</a>
            <input id="nameBox" type="text" name="nameBox" placeholder="Enter new name" maxlength="30" style="display: none;">
          </div>
          <div id="section2">
            <label id="emailBoxlbl" for="emailBox">Email: <?php echo $email; ?></label>  <a id="chgEmail" href="#" onclick="openText('emailBox')">Change Email</a>
            <input id="emailBox" type="email" name="emailBox" placeholder="Enter new email" maxlength="30" style="display: none;">
          </div>
          <div id="section3">
            <label id="formUserImagelbl" for="formUserImage">Your Image:</label>
            <img id="formUserImage" name="formUserImage" src="<?php echo $userImage;?>" alt="DefaultImage" style="width:220px;height:220px;border-radius:50%;">
            <a id="chgUImage" href="#" onclick="openUpload('imageSubmitlbl', 'imageSubmit')">Change User Image</a>
          ...
          <div id="section4">
            <label id="bannerUserImagelbl" for="bannerUserImage">Your banner image:</label>
            <img id="bannerUserImage" name="bannerUserImage" src="<?php echo $bannerImage;?>" alt="DefaultImage" style="width:400px;height:200px;">
            <a id="chgUBanner" href="#" onclick="openUpload('bannerSubmitlbl', 'bannerSubmit')">Change Banner Image</a>
            <label id="bannerSubmitlbl" for="bannerSubmit" style="display: none;">Select new banner image for your page</label>
            <input id="bannerSubmit" type="file" name="bannerSubmit" style="display: none;">
          </div>
          <div id="section5">
            <div id="bioTitle">Your Bio:</div>
            <?php
              if($about == "")
              {
                echo "<div id='bioArea'>You have not made a bio yet!</div>";
                echo "<a id='chgBio' href='#' onclick='openUpload(\"aboutBoxlbl\", \"aboutBox\")'>Make a new bio</a>";
              }
              else
              {
                echo "<div id='bioArea'> $about </div>";
                echo "<a id='chgBio' href='#' onclick='openUpload(\"aboutBoxlbl\", \"aboutBox\")'>Change bio</a>";
              }
            ?>
            <label id="aboutBoxlbl" for="aboutBox" style="display: none;">Submit a new bio:</label>
            <textarea id="aboutBox" name="aboutBox" maxlength="1000" placeholder="Enter your bio..." style="display: none;"></textarea>
          </div>
          <?php
            if(($upgrade == "1") || ($accessLevel == "2") || ($accessLevel == "1"))
            {
            }
            else
            {
              echo "<label id='requestlbl' for='request'>Click this checkbox to request to upload videos</label>";
              echo "<input id='request' type='checkbox' name='request' value='Request Video uploading'>";
            }
          ?>
