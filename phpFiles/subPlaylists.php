...
<?php
  ...
  $userID = $_GET['uID'];
  //fetch all videos that the user has uploaded
  $videoInfo = array();
  ...
  $stmt->bind_param('i', $userID);
  $stmt->execute();
  $stmt->bind_result($uploaderUsername);
  $stmt->store_result();
  $stmt->fetch();
...
function sorting()
{
  var option = document.getElementById("sortBox").value
  window.location.href= "../subRecentVideos/?uID=<?php echo $userID; ?>&sort=" + option;
}
...
<div id="container">
  <div class="center">
      ...
      <div class="vidThumbnails">
        <h2><?php echo $uploaderUsername; ?>'s Favourite's Playlist</h2>
      <?php
      if(!isset($video))
      {
        echo "<div id='thumbMessage'>lists are empty. This user has not created any lists yet</div>";
      }
      else
      {
