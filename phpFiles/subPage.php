...
<?php
  ...
  session_start();
  $uploaderUserID = $_GET['uID'];
  //retreive user information
  $stmt = $mysqli->prepare("SELECT Username, UserImage, BannerImage, Banned FROM Users WHERE UserID = ?");
  $stmt->bind_param("i", $uploaderUserID);
  $stmt->execute();
  $stmt->store_result();
  $stmt->bind_result($uploaderUsername, $uploaderThumbnail, $uploaderBanner, $banned);
  $stmt->fetch();

  $stmt->close();
  if($banned == "1" && $_SESSION['accessLevel'] != "1")
  {
    $message = "This user has been banned by administrators, we are sorry for the inconvenience";
  ...
  if($_SESSION['loggedIn']==true)
  {
    ...
    $stmt->bind_param('ii', $_SESSION['userID'], $uploaderUserID);
    $stmt->execute();
    $stmt->store_result();
    $stmt->fetch();
  ...
  //retrieve video thumbnails and infomration that correspond to the user
  $stmt = $mysqli->prepare("SELECT VideoID, VName, Thumbnail, Views, Banned FROM Videos WHERE UserID = ?");
  $stmt->bind_param("i", $uploaderUserID);
  $stmt->execute();
  $data = array();
...
<link rel="stylesheet" href="../../includes/subPage.css"/>
<script>
...
function addSub()
{
  ...
  var response = <?php echo $uploaderUserID; ?>;
...
function banFunc()
{
  //if called then send user id to ban script
  var response = <?php echo $uploaderUserID; ?>;
  var formdata = new FormData();
  formdata.append("response", response);
  var ajax = new XMLHttpRequest();
  ajax.open("POST","../../scripts/banUserScript.php");
  ajax.send(formdata);
  ajax.onreadystatechange = function()
  {
    if (ajax.readyState == 4 && ajax.status == 200)
    {
        ...
        document.getElementsByClassName("alertMessage")[0].innerHTML = "<strong>Message: </strong> This user has now been banned";
    }
  };
}
...
function undoBan()
{
  //if called then send user ID to undo ban script
  var response = <?php echo $uploaderUserID; ?>;
  var formdata = new FormData();
  formdata.append("response", response);
  var ajax = new XMLHttpRequest();
  ajax.open("POST","../../scripts/undoBanUserScript.php");
  ajax.send(formdata);
  ajax.onreadystatechange = function()
  {
    if (ajax.readyState == 4 && ajax.status == 200)
    {
      ...
      document.getElementsByClassName("alert")[0].style.display = "block";
      document.getElementsByClassName("alertMessage")[0].innerHTML = "<strong>Message: </strong> This user has now been approved for use of their account and content";
    }
  };
}
...
<div id="container">
  <div class="center">
      ...
      <div class="UserHeader">
        <div class="Himages">
          <img id="Banner" src='<?php echo $uploaderBanner; ?>' alt="BannerImage"></img>
          <img id="UserImage" src="<?php echo $uploaderThumbnail; ?>" alt="DefaultImage" style="width:220px;height:220px;border-radius:50%;">
        </div>
        <div class="nav">
          <div id="HUsername"><?php echo $uploaderUsername; ?></div>
          <div id="HSubscribe">
          <?php
            if($_SESSION['loggedIn'] == "true" && $subscribed == "false")
            {
            ...
            {
              echo "<div id='banButtonArea'>\n";
              echo "<input id='banButton' type='button' value='BAN USER' onclick='banFunc()'>\n";
              echo "</div>\n";
            }
            else if($_SESSION['accessLevel'] == "1" && $banned == "1")
            {
              echo "<div id='banButtonArea'>\n";
              echo "<input id='undoBanButton' type='button' value='UN-BAN USER' onclick='undoBan()'>\n";
              echo "</div>\n";
            }
          ?>
          </div>
          <div id="links">
            <a href="../subRecentVideos/?uID=<?php echo $uploaderUserID; ?>" id="RecentVideos">Recent Videos</a>
            <a href="../subPlaylists/?uID=<?php echo $uploaderUserID; ?>" id="Playlists">Playlists</a>
            <a href="../aboutUser?uID=<?php echo $uploaderUserID; ?>" id="About">About</a>
          </div>
        </div>
      </div>
      <div class="vidThumbnails">
        <?php
          foreach($data as $val)
          {
            if($val->banned == 0)
            {
             ...
             echo  "'> \n";
              echo  "<img alt='videoImage' class='videoImage' src=" . $val->vThumbnail  . "> \n
                     <div class='videoInfo'> <b>".  $val->vName ." </b> </div>\n
